#!/usr/bin/env sh
# PostToolUse Edit|Write -- surface a phase-file shape error on the
# write that caused it, rather than at the next review round.
#
...
cd -- "$dir" || exit 0
exec ./.claude/bin/plan guard lint
