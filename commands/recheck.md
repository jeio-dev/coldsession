...
re-review the phase end to end — /review already did, and the rev the
...
- Did any resolution create a new dependency, ordering, or scope problem?
  ...
  in the same seven-field shape /review uses, status `open`.
...
2. `.claude/bin/plan lint`, then `.claude/bin/plan recommend` and print it.
   Open findings send you back to /revise; a clean pass sends you to /approve.
